...
public class BaseSyntaxTreeFactory implements SyntaxTreeFactory {
    ...
    @Override
    public Node createSentenceMeat(final Node conjunction, final Concept opener, final List<Node> parts, final Concept closer) {
        return new SentenceMeat(conjunction, new Terminal(opener, "{"), new Terminal(closer, "}"))
                .addChildren(parts);
    }
    
    @Override
    public Node createSentenceMeatConjunction(final Concept conjunction) {
        return new SentenceMeatConjunction(new Terminal(conjunction, "Kaj"));
    }
    
    @Override
    public Node createPartSu(final Concept preposition, final Concept opener, final Node nominalSelection, final Concept closer) {
        return new PartSu(new Terminal(preposition, "Su"), new Terminal(opener, "("), new Terminal(closer, ")"))
                .addChild(nominalSelection);
    }
    
    @Override
    public Node createPartDom(final Concept preposition, final Concept opener, final Node verbalSelection, final Concept closer) {
    ...
    @Override
    public Node createVerbalSelection(final Concept verb) {
        return new VerbalSelection(verb);
    }
