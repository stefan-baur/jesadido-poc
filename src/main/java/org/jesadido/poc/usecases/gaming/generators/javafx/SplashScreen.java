...
public class SplashScreen extends SizedGameObject {
    ...
    private void init() {
        for (final Language language : Language.values()) {
            final Text titleText = new Text(this.translate(language, this.getGameModel().getTitle().getSource()));
    ...
    private String translate(final Language language, final String source) {
        if (language == Language.JI) {
            ...
            return TranslatorFactory.createTranslator(language, this.getGameModel().getGameConceptBook()).translate(source).getTranslation();
        }
    }
}
